...
module.exports = {
    ...
    if (!data) {
      return message.channel.send(
        client.emotes.error + " You do not have any site to monitor, use `-monitor` too add a website"
         );
    }
    ...
    let embed = new MessageEmbed()
      .setTitle("Send The number of the link to remove | Lütfen sayı yazın Sadece")
...
.setDescription(`Removed the website from monitoring, you can check website using \`-stats\``)
	.setColor(client.embedcolor)
.setTimestamp()
    return message.channel.send(
      hooooora
